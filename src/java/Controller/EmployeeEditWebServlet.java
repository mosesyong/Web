            ...
            try{
              ...
              HttpHost target = new HttpHost((String)session.getAttribute("url"), (Integer)session.getAttribute("port"), "http");
                ...
                if(statusCode == 202){
                    ...
                    request.setAttribute("successMsg", "Successfully Changed Data for User: " + employeeUsername);
                    request.getRequestDispatcher("UserManagement.jsp").forward(request, response);
                    return;
                }else{
                    request.setAttribute("errorMsg", "Unable to Change Access for User: " + employeeUsername);
                    request.getRequestDispatcher("UserManagement.jsp").forward(request, response);
                    return;
                }
            }catch(Exception e){
